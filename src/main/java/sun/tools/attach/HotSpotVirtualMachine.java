...
public abstract class HotSpotVirtualMachine extends VirtualMachine {
    ...
    /*
     * Load agent library If isAbsolute is true then the agent library is the absolute path to the
     * library and thus will not be expanded in the target VM. if isAbsolute is false then the agent
     * library is just a library name and it will be expended in the target VM.
     */
    private void loadAgentLibrary(String agentLibrary, boolean isAbsolute, String options) throws AgentLoadException,
     ...
     */
    public void loadAgentLibrary(String agentLibrary, String options) throws AgentLoadException,
            AgentInitializationException, IOException {
     ...
     */
    public void loadAgentPath(String agentLibrary, String options) throws AgentLoadException,
            AgentInitializationException, IOException {
     ...
     */
    public void loadAgent(String agent, String options) throws AgentLoadException, AgentInitializationException,
            IOException {
        String args = agent;
        if (options != null) {
            args = args + "=" + options;
        ...
        try {
            ...
            /*
             * Translate interesting errors into the right exception and message (FIXME: create a
             * better interface to the instrument implementation so this isn't necessary)
             */
    ...
    /*
     * Utility method to read an 'int' from the input stream. Ideally we should be using
     * java.util.Scanner here but this implementation guarantees not to read ahead.
     */
    ...
    /*
     * Return attach timeout based on the value of the sun.tools.attach.attachTimeout property, or
     * the default timeout if the property is not set to a positive value.
     */
    long attachTimeout() {
        if (attachTimeout == 0) {
            synchronized (this) {
