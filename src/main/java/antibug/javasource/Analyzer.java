 ...
 * notice.</b>
 */
public class Analyzer extends JCTree.Visitor {
    ...
    /**
     * A table mapping trees to their documentation comments (can be null)
     */
     ...
     * @param output
     */
    public Analyzer(Writer writer, SourceMapper mapper) throws Exception {
        this.out = writer;
    ...
    public void printExpr(JCTree tree, int prec) throws IOException {
        int actualLine = mapper.getLine(tree);

        while (logicalLine < actualLine) {
            print(mapper.readLineFrom(logicalLine));
            println();
            align();
        }
    ...
    public void printFlags(long flags) throws IOException {
        ...
        if ((flags & com.sun.tools.javac.code.Flags.ANNOTATION) != 0) print("@");
    }
    ...
    /**
     * If type parameter list is non-empty, print it enclosed in {@literal "<...>"} brackets.
     */
    ...
    public void visitMethodDef(JCMethodDecl tree) {
        try {
            ...
            if (tree.name == tree.name.table.names.init && enclClassName == null && sourceOutput) return;
            printDocComment(tree);
            printExpr(tree.mods);
            printTypeParameters(tree.typarams);
        ...
        try {
            ...
            if (tree.def != null) {
                ...
                enclClassName = tree.def.name != null ? tree.def.name
                        : tree.type != null && tree.type.tsym.name != tree.type.tsym.name.table.names.empty ? tree.type.tsym.name
                                : null;
