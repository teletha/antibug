 ...
 * {@link BlockJUnit4ClassRunner}, {@code T} is {@link Method} . For {@link Suite}, {@code T} is
 * {@link Class}.) Subclasses must implement finding the children of the node, describing each
 * child, and running each child. ParentRunner will filter and sort children, handle
...
public abstract class ParentRunner<T> extends Runner implements Filterable, Sortable, TestRule {
    ...
    /**
     * Runs the test corresponding to {@code child}, which can be assumed to be an element of the
     * list returned by {@link ParentRunner#getChildren()}. Subclasses are responsible for making
     * sure that relevant test events are reported through {@code notifier}
     */
     ...
     * {@link #getTestClass()}). Default implementation adds an error for each method annotated with
     * {@code @BeforeClass} or {@code @AfterClass} that is not {@code public static void} with no
     * arguments.
     */
    ...
    /**
     * Constructs a {@code Statement} to run all of the tests in the test class. Override to add
     * pre-/post-processing. Here is an outline of the implementation:
     * <ol>
     * <li>Determine the children to be run using {@link #getChildren()} (subject to any imposed
     * filter and sort).</li>
     * <li>If there are any children remaining after filtering and ignoring, construct a statement
     * that will:
     * <ol>
     * <li>Apply all {@code ClassRule}s on the test-class and superclasses.</li>
     * <li>Run all non-overridden {@code @BeforeClass} methods on the test-class and superclasses;
     * if any throws an Exception, stop execution and pass the exception on.</li>
     * <li>Run all remaining tests on the test-class.</li>
     * <li>Run all non-overridden {@code @AfterClass} methods on the test-class and superclasses:
     * exceptions thrown by previous steps are combined, if necessary, with exceptions from
     * AfterClass methods into a {@link org.junit.runners.model.MultipleFailureException}.</li>
     * </ol>
     * </li>
     * </ol>
     *
     ...
     * from AfterClass methods into a {@link org.junit.runners.model.MultipleFailureException}.
     */
    ...
    /**
     * Returns a {@link Statement}: apply all static fields assignable to {@link TestRule} annotated
     * with {@link ClassRule}.
     *
    ...
    /**
     * @return the {@code ClassRule}s that can transform the block that runs each method in the
     *         tested class.
     */
    ...
    /**
     * Returns a {@link Statement}: Call {@link #runChild(Object, RunNotifier)} on each object
     * returned by {@link #getChildren()} (subject to any imposed filter and sort)
     */
     ...
     * <code>false</code>.
     * <p>
     * {@link BlockJUnit4ClassRunner}, for example, overrides this method to filter tests based on
     * the {@link Ignore} annotation.
     */
