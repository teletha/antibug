...
public abstract class ParentRunner<T> extends Runner implements Filterable, Sortable, TestRule {
     ...
     * <ul>
     * <li>Call {@link #runChild(Object, RunNotifier)} on each object returned by
     * {@link #getChildren()} (subject to any imposed filter and sort).</li>
     * <li>ALWAYS run all non-overridden {@code @BeforeClass} methods on this class and superclasses
     * before the previous step; if any throws an Exception, stop execution and pass the exception
     * on.
     * <li>ALWAYS run all non-overridden {@code @AfterClass} methods on this class and superclasses
     * before any of the previous steps; all AfterClass methods are always executed: exceptions
     * thrown by previous steps are combined, if necessary, with exceptions from AfterClass methods
     * into a {@link MultipleFailureException}.
     * </ul>
     * 
     * @param notifier
    ...
    private List<T> getFilteredChildren() {
        if (fFilteredChildren == null) fFilteredChildren = new ArrayList<T>(getChildren());
