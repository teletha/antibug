...
public class TestClass {
    ...
    public TestClass(Class<?> klass) {
        ...
        // addToAnnotationLists(new PowerAssertRule(PowerAssert.class), fFieldsForAnnotations);
    }
    ...
    private <T extends FrameworkMember<T>> void addToAnnotationLists(T member, Map<Class<?>, List<T>> map) {
        for (Annotation each : member.getAnnotations()) {
            ...
            if (member.isShadowedBy(members)) return;
    ...
    private <T> List<T> getAnnotatedMembers(Map<Class<?>, List<T>> map, Class<? extends Annotation> type) {
        if (!map.containsKey(type)) map.put(type, new ArrayList<T>());
        return map.get(type);
    }
    ...
    public Annotation[] getAnnotations() {
        if (fClass == null) return new Annotation[0];
    ...
    public <T> List<T> getAnnotatedFieldValues(Object test, Class<? extends Annotation> annotationClass, Class<T> valueClass) {
        ...
        for (FrameworkField each : getAnnotatedFields(annotationClass)) {
            try {
                ...
                if (valueClass.isInstance(fieldValue)) results.add(valueClass.cast(fieldValue));
